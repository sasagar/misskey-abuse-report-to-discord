...
const checkAbuse = async () => {
	...
	try {
		await cli
			.request("admin/abuse-user-reports", args)
			.then(async (arr) => {
				...
				for (let i = 0; i < len; i++) {
					...
					const jstString = formatTZ(jstDate, "PPP(EEE) HH:mm:ss (xxx)", {
						locale: ja,
						timeZone: "Asia/Tokyo",
					});
					//=> "2000-01-01 09:00:00"
						...
						embeds: [
							{
								...
								footer: {
									text: "いかすきー",
									icon_url:
										"https://ikaskey-s3.bktsk.com/ikaskey/a1a91699-6dbe-487d-adef-0e1e6442ef66.png",
								},
							},
						],
					};
